...
class AirCargoProblem(Problem):
    ...
    def result(self, state: str, action: Action):
        ...
        # Cycle through all the fluents that are possible from the old/current state (fluents are concatenations of literals
        # i.e. At(C1,JFK) - note that fluents can change over time since C1 might not always be at JFK). For each fluent we 
        # check that it is preserved by the action that is fed into the result function (i.e. make sure it is not in
        # effect_removed). If so, we append it to the new state. Fluents at the new_state can originate in 4 different 
        # ways (hence the 4 loops below) and we ensure that all are added.
        new_state = FluentState([], [])
        old_state = decode_state(state, self.state_map)
